...
export default function BrowsePokedex() {
  ...
  // For better performance, we'll render a fixed number per generation
  // and let users load more if needed
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-4">
              {pokemonData[genId].map(pokemon => (
                <Link key={pokemon.id} href={`/pokemon/${pokemon.name}`}>
                  <a className="bg-gray-800 hover:bg-gray-700 rounded-lg p-4 transition-transform hover:scale-105">
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <Head>
        <title>Browse Pokédex | Pokédex Live</title>
        <meta name="description" content="Browse all Pokémon in the Pokédex" />
      </Head>
      
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-6">Browse Pokédex</h1>
